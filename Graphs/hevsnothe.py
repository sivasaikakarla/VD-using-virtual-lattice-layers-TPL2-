...
def apply_histogram_equalization(frame, roi_x, roi_y, roi_width, roi_height):
    roi = frame[roi_y:roi_y + roi_height, roi_x:roi_x + roi_width]
    roi_yuv = cv2.cvtColor(roi, cv2.COLOR_BGR2YUV)
    hist_before = cv2.calcHist([roi_yuv], [0], None, [256], [0, 256])

    roi_yuv[:, :, 0] = cv2.equalizeHist(roi_yuv[:, :, 0])
    equalized_roi = cv2.cvtColor(roi_yuv, cv2.COLOR_YUV2BGR)
    frame[roi_y:roi_y + roi_height, roi_x:roi_x + roi_width] = equalized_roi

    hist_after = cv2.calcHist([roi_yuv], [0], None, [256], [0, 256])
    return frame, hist_before, hist_after

def plot_histogram(hist_before, hist_after):
    plt.figure(figsize=(8, 5))
    plt.plot(hist_before, color='b', label='Before Equalization')
    plt.plot(hist_after, color='r', label='After Equalization')
    plt.xlim([0, 256])
...
def main():
    ...
    while cap.isOpened():
        ...
        if frame1.shape[:2] == frame2.shape[:2]:
            # Apply histogram equalization to ROIs
            frame1, hist1_before, hist1_after = apply_histogram_equalization(frame1, roi1_x, roi1_y, roi1_width, roi1_height)
            # frame2, hist2_before, hist2_after = apply_histogram_equalization(frame2, roi2_x, roi2_y, roi2_width, roi2_height)

            plot_histogram(hist1_before, hist1_after)
            # plot_histogram(hist2_before, hist2_after)
