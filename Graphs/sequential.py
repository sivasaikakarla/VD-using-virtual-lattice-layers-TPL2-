...
if __name__ == '__main__':
    ...
    frames = list(range(1, len(frame_times) + 1))
    plt.figure(figsize=(12, 6))
    plt.subplot(1, 2, 1)
    plt.plot(frames, frame_times, marker='o')
    plt.xlabel('Frame Number')
    plt.ylabel('Execution Time (seconds)')
    plt.title('Execution Time per Frame')
    plt.grid(True)

    plt.subplot(1, 2, 2)
    plt.plot(frames, memory_usages, marker='o', color='orange')
    plt.xlabel('Frame Number')
    plt.ylabel('Memory Usage (MB)')
    plt.title('Memory Usage per Frame')
    plt.grid(True)
