...
# 419, 226, 419, 148
roi1_x, roi1_y, roi1_width, roi1_height = 419, 226, 419, 148
...
def apply_preprocessing(frame, roi_x, roi_y, roi_width, roi_height, channel_choice):
    roi = frame[roi_x:roi_x + roi_width, roi_y:roi_y + roi_height]
    ...
    if channel_choice == 'gray':
        ...
        roi_preprocessed = cv2.cvtColor(roi_clahe, cv2.COLOR_GRAY2BGR)
    else:
        ...
        roi_preprocessed = cv2.cvtColor(hsv_clahe, cv2.COLOR_HSV2BGR)
    frame[roi_x:roi_x + roi_width, roi_y:roi_y + roi_height] = roi_preprocessed
    return frame
...
def process_hsv(frame1, frame2, channels, channel_choice):
    frame1 = apply_preprocessing(frame1, roi1_x, roi1_y, roi1_width, roi1_height, channel_choice)
    frame2 = apply_preprocessing(frame2, roi1_x, roi1_y, roi1_width, roi1_height, channel_choice)
